...
class ADHDAnalyzer {
    ...
    fun analyzePerformance(
        ...
        // Only add response time marker if it's meaningful (above minimum threshold)
        ...
        // Only add variability marker if it's meaningful
        if (responseTimeVariability > 50) {
            markers.add(BehavioralMarker(
        ...
        // Only add accuracy marker if it's below expected levels
        if (accuracy < 85) {
            markers.add(BehavioralMarker(
                name = "Task Accuracy",
        ...
        // Only add missed responses marker if it's meaningful
        if (missedResponseRate > 5) {
            markers.add(BehavioralMarker(
        ...
        // Only add look away marker if it's above minimal threshold
        if (lookAwayRate > 2) {
            markers.add(BehavioralMarker(
                name = "Look Away Rate",
                value = lookAwayRate,
        ...
        // Add sustained attention marker - use significance thresholds
        if (faceMetrics.sustainedAttentionScore < 70) {
            markers.add(BehavioralMarker(
        ...
        // Add look away duration marker only if meaningful
        if (faceMetrics.averageLookAwayDuration > 1500) {
            markers.add(BehavioralMarker(
        ...
        // Add attention lapses marker only if meaningful
        if (faceMetrics.attentionLapseFrequency > 2) {
            markers.add(BehavioralMarker(
        ...
        // Add distractibility marker only if meaningful
        if (faceMetrics.distractibilityIndex > 30) {
            markers.add(BehavioralMarker(
        ...
        // Add blink rate marker only if significantly elevated
        if (faceMetrics.blinkRate > 30) {
            markers.add(BehavioralMarker(
                name = "Blink Rate",
        ...
        // Add face visibility marker only if it's poor
        if (faceMetrics.faceVisiblePercentage < 80) {
            markers.add(BehavioralMarker(
        ...
        // Add facial movement marker only if significant
        if (faceMetrics.facialMovementScore > 50) {
            markers.add(BehavioralMarker(
        ...
        // Add emotion changes marker only if significant
        if (normalizedEmotionChanges > 40) {
            markers.add(BehavioralMarker(
        ...
        // Add emotion variability marker only if significant
        if (faceMetrics.emotionVariabilityScore > 50) {
            markers.add(BehavioralMarker(
        ...
        val fidgetingFactor = when {
            ...
            else -> 0 // Changed from 10 to 0 for when fidgeting is very low
        }

        // Only add fidgeting marker if it's above minimal threshold
        if (motionMetrics.fidgetingScore > 10) {
            markers.add(BehavioralMarker(
                name = "Fidgeting Score",
                value = motionMetrics.fidgetingScore.toFloat(),
                threshold = 65f,
                significance = if (motionMetrics.fidgetingScore > 75) 3
                else if (motionMetrics.fidgetingScore > 60) 2
                else if (motionMetrics.fidgetingScore > 30) 1
                else 0, // No significance for very low fidgeting
        ...
        // Only add direction changes marker if above minimal threshold
        if (normalizedDirectionChanges > 30) {
            markers.add(BehavioralMarker(
        ...
        // Only add sudden movements marker if above minimal threshold
        if (normalizedSuddenMovements > 30) {
            markers.add(BehavioralMarker(
        ...
        val restlessnessFactor = when {
            ...
            else -> 0 // Changed from 10 to 0 for very low restlessness
        }

        // Only add restlessness marker if significant
        if (motionMetrics.restlessness > 10) {
            markers.add(BehavioralMarker(
                name = "Restlessness",
                value = motionMetrics.restlessness.toFloat(),
                threshold = 65f,
                significance = if (motionMetrics.restlessness > 75) 3
                else if (motionMetrics.restlessness > 60) 2
                else if (motionMetrics.restlessness > 30) 1
                else 0, // No significance for very low restlessness
        ...
        val adhdProbabilityScore = ((attentionScore * 0.50f) +
                (hyperactivityScore * 0.25f) +
    ...
    private fun calculateConfidenceLevel(
        ...
        // Base confidence at 60 (moderate) instead of 75 (high)
        var confidence = 60
        ...
        // Analyze marker significance patterns - count markers with significance >= 2
        val significantMarkers = markers.count { it.significance >= 2 }
        confidence += when {
            significantMarkers >= 8 -> 15  // Many strong indicators
            significantMarkers >= 5 -> 10  // Multiple strong indicators
            significantMarkers >= 3 -> 5   // Some strong indicators
            significantMarkers >= 1 -> 0   // At least one strong indicator
            markers.size >= 5 -> -5       // Markers exist but none are strong
            else -> -15                    // Very few markers of any significance
        }
